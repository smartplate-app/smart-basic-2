...
Deno.serve(async (req) => {
  try {
    ...
    const connections = await base44.asServiceRole.entities.POSConnection.filter({ is_active: true });
    let count = 0;
    for (const connection of connections) {
      try {
        await performSync(base44, connection);
        count++;
      } catch (e) {
        console.error(`Failed to sync for ${connection.created_by}:`, e);
      }
    }
