...
Deno.serve(async (req) => {
  try {
    ...
    const prompt = `קרא את כל התפריט המצורף (תמונה או PDF) וחלץ את כל שמות המנות.
...
המשימה שלך:
1. עבור על כל התפריט (כל העמודים).
2. חלץ את השם של כל מנה (הטקסט לפני המחיר בשורה הראשונה של כל פריט).
3. התעלם מהמחירים ומהתיאורים.
4. החזר אובייקט JSON עם מערך 'menu_items' המכיל את רשימת השמות המלאה.
