  ...
  const loadData = async (currentUser, retryCount = 0) => {
                try {
                  ...
                  if (isStoreUser && storeOwnerEmail) {
                    ...
                    const promises = [
                      base44.entities.Supplier.filter({ $or: [{ created_by: storeOwnerEmail }, { "data.created_by": storeOwnerEmail }] }, '-created_date'),
                      base44.entities.Supplier.filter({ store_owner_email: storeOwnerEmail }, '-created_date'),
                      base44.entities.Supplier.filter({ $or: [{ created_by: workingEmail }, { "data.created_by": workingEmail }] }, '-created_date'),
                      base44.entities.Supplier.filter({ store_owner_email: workingEmail }, '-created_date'),
                      base44.entities.Item.filter({ $or: [{ created_by: storeOwnerEmail }, { "data.created_by": storeOwnerEmail }] }),
                      base44.entities.Item.filter({ store_owner_email: storeOwnerEmail }),
                      base44.entities.Item.filter({ $or: [{ created_by: workingEmail }, { "data.created_by": workingEmail }] }),
                      base44.entities.Item.filter({ store_owner_email: workingEmail })
                    ];
                    if (headEmail) {
                      promises.push(
                        base44.entities.Supplier.filter({ $or: [{ created_by: headEmail }, { "data.created_by": headEmail }] }, '-created_date'),
                        base44.entities.Item.filter({ $or: [{ created_by: headEmail }, { "data.created_by": headEmail }] })
                      );
                    }
  ...
  const generateReport = async () => {
    try {
      ...
      const [allOrders, allReceipts] = await Promise.all([
        base44.entities.Order.filter({ $or: [{ created_by: workingEmail }, { "data.created_by": workingEmail }] }),
        base44.entities.SupplyReceipt.filter({ $or: [{ created_by: workingEmail }, { "data.created_by": workingEmail }] })
      ]);
