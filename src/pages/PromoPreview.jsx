...
export default function PromoPreview() {
  ...
  const t = {
    he: {
      ...
      previewTitle: "תצוגת מסך התחברות"
    },
    en: {
      ...
      previewTitle: "Login Screen Preview"
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setLoading(false);
      alert("This is just a preview page to show the graphics. The actual login works normally!");
    }, 1500);
  };

  return (
    <div className="p-0 sm:p-6 bg-slate-100 min-h-screen">
      <div className="mb-6 flex justify-between items-center px-4 pt-4 sm:px-0 sm:pt-0">
        <h1 className="text-xl sm:text-2xl font-bold">{text.previewTitle}</h1>
      ...
      <div className={`sm:rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row border border-slate-200 ${isRTL ? 'font-sans' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
        {/* Marketing Section */}
        <div className="hidden md:flex md:w-3/5 lg:w-3/4 bg-gradient-to-br from-[#1b4332] to-[#2d6a4f] text-white p-8 lg:p-16 flex-col justify-between relative overflow-hidden min-h-[800px]">
                ...
                <div className="grid grid-cols-2 gap-4 absolute inset-0 transform translate-x-12 -translate-y-8">
                  <div className="space-y-4 pt-12">
                    <div className="bg-white p-2 rounded-2xl shadow-2xl transform -rotate-2 hover:rotate-0 transition-transform duration-500">
                      <img src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=600&q=80" alt={text.imgAlt1} className="w-full h-48 object-cover rounded-xl" />
                    </div>
                    <div className="bg-white p-2 rounded-2xl shadow-2xl transform rotate-3 hover:rotate-0 transition-transform duration-500">
                      <img src="https://images.unsplash.com/photo-1581349485608-9469926a8e5e?auto=format&fit=crop&w=600&q=80" alt={text.imgAlt2} className="w-full h-56 object-cover rounded-xl" />
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="bg-white p-2 rounded-2xl shadow-2xl transform rotate-2 hover:rotate-0 transition-transform duration-500">
                      <img src="https://images.unsplash.com/photo-1613511874284-cd4a3b8d9ba8?auto=format&fit=crop&w=600&q=80" alt={text.imgAlt3} className="w-full h-64 object-cover rounded-xl" />
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Login Section */}
        <div className="w-full md:w-2/5 lg:w-1/4 min-w-[320px] max-w-[500px] bg-white flex flex-col min-h-[800px] relative overflow-y-auto z-20">
          ...
          <div className="flex-1 flex flex-col justify-center px-8 sm:px-12 lg:px-16 py-12">
            ...
            <div className="mt-8 text-center text-slate-600 text-sm">
              {text.noAccount}{" "}
              <span className="text-[#2d6a4f] font-bold cursor-pointer hover:text-[#1b4332] hover:underline">
                {text.createOne}
              </span>
            </div>
          </div>
