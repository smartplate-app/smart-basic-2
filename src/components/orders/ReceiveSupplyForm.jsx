            ...
            <>
              {order && (
                <>
                            ...
                            <div key={idx} className="flex items-center gap-2 text-sm">
                              ...
                              {change.old_price !== change.new_price && (
                                <Badge variant="outline" className={getChangeBadgeColor(change.change_type)}>
                                  {t('price')}: {change.old_price.toFixed(2)} → {change.new_price.toFixed(2)}
                                  ({change.price_change_percent > 0 ? '+' : ''}{change.price_change_percent.toFixed(1)}%)
                                </Badge>
                              )}
                              {change.old_discount !== change.new_discount && (
                                <Badge variant="outline" className={getChangeBadgeColor(change.change_type)}>
                                  {t('discount')}: {change.old_discount}% → {change.new_discount}%
                                </Badge>
                              )}
                            </div>
                          ))}
                  ...
                  {/* Display manual input fields for invoice details after scan */}
                  <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-4">
                    <h3 className="font-bold text-blue-900 mb-3 flex items-center gap-2">
                      <Scan className="w-5 h-5" />
                      {t('invoice_details') || 'פרטי חשבונית'}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <Label className="text-lg font-semibold">{t('verify_items')}</Label>
                    </div>
                    <div className="space-y-3">
                      {formData.verified_items.map((item, index) => (
                        <Card key={index} className={item.price_changed || item.discount_changed ? 'border-orange-300 bg-orange-50' : ''}>
                          <CardContent className="pt-4">
                            <div className="flex flex-col gap-3">
                              <div className="flex items-center justify-between">
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                <div>
                                  <Label className="text-xs">
                                    {t('price')}
                                    {item.catalog_price > 0 && ` (קטלוג: ${item.catalog_price.toFixed(2)})`}
                                  </Label>
                                  <Input
                                    type="number"
                                    step="0.01"
                                    value={item.actual_price}
                                    onChange={(e) => updateVerifiedItem(index, 'actual_price', parseFloat(e.target.value) || 0)}
                                    className={item.price_changed ? 'border-orange-400' : ''}
                                  />
                                </div>
                                <div>
                                  <Label className="text-xs">
                                    {t('discount')} %
                                    {item.catalog_discount > 0 && ` (קטלוג: ${item.catalog_discount}%)`}
                                  </Label>
                                  <Input
                                    type="number"
                                    step="0.01"
                                    ...
                                    value={item.actual_discount}
                                    onChange={(e) => updateVerifiedItem(index, 'actual_discount', parseFloat(e.target.value) || 0)}
                                    className={item.discount_changed ? 'border-orange-400' : ''}
                                  />
                                </div>
                              </div>
