          ...
          {(noOrderMode || order) ? (
            <>
              <div className="space-y-2">
                <Popover open={supplierPopoverOpen} onOpenChange={setSupplierPopoverOpen} modal={false}><PopoverTrigger asChild><Button variant="outline" role="combobox" aria-expanded={supplierPopoverOpen} className="w-full justify-between font-normal text-black"><span className="truncate text-black">{formData.supplier_id && availableSuppliers.length > 0 ? availableSuppliers.find((s) => s.id === formData.supplier_id)?.name || t('select_supplier') : t('select_supplier')}</span><ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" /></Button></PopoverTrigger><PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start"><Command><CommandInput placeholder={language === 'he' ? 'חפש ספק...' : 'Search supplier...'} autoFocus /><CommandList><CommandEmpty>{t('no_suppliers') || 'לא נמצאו ספקים.'}</CommandEmpty><CommandGroup>
