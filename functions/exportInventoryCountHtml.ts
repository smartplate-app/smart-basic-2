...
Deno.serve(async (req) => {
  try {
    ...
    const signatureDataUrl = payload?.signature_data_url || '';
    if (!countId) {
...
<body>
  <div class="page">
    ...
    <section class="signature">
      ...
      ${signatureDataUrl
        ? `<img src="${esc(signatureDataUrl)}" alt="Signature" style="max-width:100%;height:140px;border:1px dashed var(--border);border-radius:8px;background:#fff;" />`
        : `<div class="meta">לא סופקה חתימה</div>`
      }
    </section>
